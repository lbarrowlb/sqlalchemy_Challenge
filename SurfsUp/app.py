...
def date_prev_year():
 ...
 # Use the most recent date to calculate the date one year from the last date
...
def homepage():
    ...
    <li>To retrieve the minimum, average, and maximum temperatures for a specific start date, use <strong>/api/v1.0/stats/&ltstart&gt</strong>
    (replace start date in yyyy-mm-dd format)</li>
    <li>To retrieve the minimum, average, and maximum temperatures for a specific start-end range, use
    <strong>/api/v1.0/stats/&ltstart&gt/&ltend&gt</strong> (replace start and end date in yyyy-mm-dd format)</li>
    </ul>
    """
...
def cal_temp(start, end=None):
    ...
    # Convert list of tuples into normal list
    data_list = list(np.ravel(raw_data))
